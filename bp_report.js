//#region bp_report.js  version 2
// 這段程式碼是用來從 Google Apps Script Web App 獲取血壓報告數據，並在網頁上顯示表格和圖表的功能。
...
document.addEventListener('DOMContentLoaded', () => { // 當 DOM 完全加載後執行
  fetch(apiURL) // 發送 GET 請求到 Google Apps Script Web App
    ...
    .then(data => { // 解析 JSON 數據
      ...
      displayHeaders.forEach(header => { // 創建表頭元素並加入到表頭行中
      ...
      allRecords = data.records.map(row => { 
        const displayRow = row.slice(0, 7);
        if (displayRow[0]) { // 確保日期存在
          let dateObj = new Date(displayRow[0]); // 轉換為日期物件
          let month = String(dateObj.getMonth() + 1).padStart(2, "0"); // 獲取月份 (getMonth() 會比實際月份少 1，所以要 +1)
          let day = String(dateObj.getDate()).padStart(2, "0"); // 獲取日期
          displayRow[0] = `${month}/${day}`; // 正確格式化 MM/DD
        }
        return displayRow;
      });
...
document.getElementById("bpForm").addEventListener("submit", function(event) {
    ...
    submitButton.disabled = true; // 禁用按鈕，避免多次點擊
    ...
    .then(result => {
        alert("數據已成功儲存到 Google 試算表！");
        document.getElementById("bpForm").reset(); // 清空表單
        setTimeout(() => { submitButton.disabled = false; }, 3000); // 3 秒後重新啟用按鈕
    })
    .catch(error => console.error("發生錯誤: ", error));
});
...
// 切換到下一頁
function nextPage() {
  if (currentPage < Math.ceil(allRecords.length / itemsPerPage)) {
    currentPage++;
    renderTable();
  }
}
...
function generateChart() {
  ...
  const systolicData = [];
  const diastolicData = [];
  const last7Records = allRecords.slice(-7);

  last7Records.forEach(row => {
    labels.push(row[0]);
    systolicData.push(parseInt(row[1]));
    diastolicData.push(parseInt(row[2]));
  });
  ...
  labels.push("平均");
  systolicData.push(avgSystolic);
  diastolicData.push(avgDiastolic);
